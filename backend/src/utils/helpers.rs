...
pub fn hashed_or_verity_pass(password: &str, username: &str, method: &str, db_hashed_pass: Option<&str> ) -> String {
    ...
    match method {
        ...
        "verify_hash" => {
            match db_hashed_pass {
                Some(hash) => {
                    let verified_hash: bool = argon2::verify_encoded(&hash, &strong_pass).unwrap();
                    match verified_hash {
